<?php 
  ...
  $query = "SELECT * FROM custom_member_details where status='Pending'  order by id ASC limit 30";
...
// $pdf->Cell(0,-10, '***This is a computer-generated receipt. No signature is required.***', 0, 0, 'C');
      ...
      array_push($files,$data);
        ...
        echo 'window.location.href = "custom_outbox.php";';
        echo '</script>';  
      ...
      function sendEmail($files,$i){
        ...
        $mail->Password = 'Ummc@1968';   // SMTP password 
        ...
        $bodyContent .= '<p></p>'; 
        $bodyContent .= '<p>Receipt payment hereby attached for your perusual.</p>'; 
        $bodyContent .= '<p></p>'; 
        $bodyContent .= '<p></p>'; 
	...
	$bodyContent .= '<p></p>';
        ...
        if(!$mail->send()) { 
          ...
          $total = $files[$i]['total'];
          include 'config.php'; 
          $query1 = "UPDATE custom_member_details set status='Done',total = '$total' where d_no=$status";
