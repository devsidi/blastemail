    ...
    if(!empty($_FILES['file']['name']) && in_array($_FILES['file']['type'], $csvMimes)){
        ...
        if(is_uploaded_file($_FILES['file']['tmp_name'])){
            ...
            while(($line = fgetcsv($csvFile)) !== FALSE){
                ...
                $prevQuery = "SELECT d_no FROM custom_member_details WHERE d_no = '".$line[6]."' AND status = 'Pending'";
                $prevResult = $con->query($prevQuery);
                $count =  mysqli_num_rows($prevResult);

                if($count > 0){
                    // Update member data in the database
                    $con->query("UPDATE custom_member_details SET name = '".$name."', email = '".$email."', staff_no = '".$staff_no."', unit_ward = '".$unit_ward."', icno = '".$icno."',
                    ...
                    $con->query("INSERT INTO custom_member_details (membership_no,name, email, staff_no,unit_ward,icno,d_no,entry_fee,fee,lifetime_fee,donation,others,year,date_receipt, created_at, status) 
...
<div class=" text-black" style="margin-top:50px">
  ...
  <div class="card-body">
    ...
    <button type="submit" class="btn btn-warning" name="submit" value="submit">Upload</button><br><br>
<p>After upload then can check the custom blast email</p>
